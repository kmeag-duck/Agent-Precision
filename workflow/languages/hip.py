...
11. Kernel timing. Repeat the kernel launch N=11 times: 1 untimed
    ...
    or JSON emission. Use hipEvent for GPU timing:
    `hipEvent_t start, stop; hipEventCreate(&start); hipEventCreate(&stop);`
    then per trial, `hipEventRecord(start)`, launch the kernel,
    `hipEventRecord(stop)`, `hipEventSynchronize(stop)`,
    `float ms; hipEventElapsedTime(&ms, start, stop);`. Convert to
