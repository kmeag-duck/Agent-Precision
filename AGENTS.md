...
- `workflow/orchestrator.py` — itself a Claude conversation. Tools are one-per-agent-type (`spawn_precision_advisor`, `spawn_analyst`, `spawn_rewriter`, `spawn_verifier`, `spawn_baseline_harness`), plus four deterministic (non-LLM) tools defined in `workflow/tools.py` (`compile_baseline_driver`, `run_baseline_driver`, `splice_rewritten_kernel`, `compile_rewritten_driver`), plus `finish`. Contains the **human-in-the-loop pause** (`_hitl_pause`): every tool call is shown and the user must approve `y/n/q` before it runs. This pause is the whole point of v0 — do not remove it or auto-approve. A `MAX_TURNS=40` backstop guards against runaway loops if the HITL is left unattended (raised from 20 when the splice + compile_rewritten steps landed); don't raise it casually.
...
The orchestrator system prompt enforces the pipeline `(precision_advisor ->)? analyst -> rewriter -> verifier -> finish`, and explicitly forbids calling `finish` unless the most recent `spawn_verifier` returned `verdict='accept'`. It also forbids calling `spawn_precision_advisor` when the user supplied a tolerance on the command line, and forbids calling it more than once. `spawn_baseline_harness` is orthogonal to this pipeline — it is a side artifact (see next section), never a precondition for `finish`, and called at most once per run, only for Kokkos `.cpp` inputs. `compile_baseline_driver`, `run_baseline_driver`, `splice_rewritten_kernel`, and `compile_rewritten_driver` form a chain off `spawn_baseline_harness`: each is the deterministic follow-up to the previous step, each is called at most once per run with the same `kernel_stem`, and a failure or skip at any link prevents (but does not require) the next. `splice_rewritten_kernel` additionally requires the most recent `spawn_verifier` to have returned `verdict='accept'` — it takes the rewriter's output and splices it between the sentinel lines in `baselines/<stem>/driver.cpp` to produce `baselines/<stem>/rewritten/driver.cpp`. None of these four deterministic tools' results are preconditions for `finish`. Those rules live in the prompt, not in code — the orchestrator is trusted to obey them. If you add a hard guard, do it in `orchestrator.py` and keep the prompt in sync.
...
- **Dynamic verification stack.** The current `verifier` agent only checks faithfulness of the rewrite to the analyst's verdict (a static / textual check). Mechanical verification is partially landed: `compile_baseline_driver`, `run_baseline_driver`, `splice_rewritten_kernel`, and `compile_rewritten_driver` are all implemented as deterministic orchestrator tools (see "Baseline harness" above). What remains is a `run_rewritten_driver` tool (analogous to `run_baseline_driver`, with cwd `baselines/<stem>/rewritten/`, producing its own `reference.json`) and a comparator tool that diffs the two `reference.json` files against the run's tolerance. Both should keep the `{status, stdout, stderr, artifacts}` shape shared with the existing four deterministic tools, so the orchestrator's tool-result handling doesn't change when "compile / run locally" eventually becomes "submit job and poll" (see "JLSE/async toolchain" below). `MAX_TURNS` was already raised from 20 to 40 to make room for this chain; landing run_rewritten + compare should fit inside that budget.
