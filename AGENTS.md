...
The orchestrator system prompt enforces the pipeline `(precision_advisor ->)? analyst -> rewriter -> verifier -> finish`, and explicitly forbids calling `finish` unless the most recent `spawn_verifier` returned `verdict='accept'`. It also forbids calling `spawn_precision_advisor` when the user supplied a tolerance on the command line, and forbids calling it more than once. `spawn_baseline_harness` is orthogonal to this pipeline — it is a side artifact (see next section), never a precondition for `finish`, and called at most once per run, only for Kokkos `.cpp` inputs. Those rules live in the prompt, not in code — the orchestrator is trusted to obey them. If you add a hard guard, do it in `orchestrator.py` and keep the prompt in sync.
